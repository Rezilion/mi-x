...
def check_kpatch(debug, container_name):
    ...
    if modules:
        ...
        for kpatch in KPATCH_MODULE_NAMES:
            if modules.__contains__(kpatch):
                print(constants.FULL_NEGATIVE_RESULT_MESSAGE)
...
def check_release(debug, container_name):
    ...
    if host_information == constants.UNSUPPORTED:
        ...
        print(constants.FULL_EXPLANATION_MESSAGE.format(f'Can not determine vulnerability status, no distribution and '
                                                        f'version values'))
    return ''
    ...
    if os_type.linux(debug, container_name):
        ...
        if fixed_release == constants.UNSUPPORTED:
            ...
            if check_kernel_version == constants.UNSUPPORTED:
                ...
                if fixed_release.__contains__('Red'):
